...
dataleft=zeros(21,22);
dataleft(1:7,:)=datA(21:27,:);
dataleft(8:21,:)=datB(41:54,:);
classt=zeros(21,1);
classt(1:7,1)=1;
classt(8:21,1)=2;
...
data_selected=zeros(80,12);
data_selected(:,1:2)=dataall(:,1:2);
data_selected(:,3:4)=dataall(:,8:9);
data_selected(:,5:9)=dataall(:,12:16);
data_selected(:,10)=dataall(:,20);
data_selected(:,11)=dataall(:,18);
data_selected(:,12)=dataall(:,22);


data_selected_t=zeros(21,12);
data_selected_t(:,1:2)=dataleft(:,1:2);
data_selected_t(:,3:4)=dataleft(:,8:9);
data_selected_t(:,5:9)=dataleft(:,12:16);
data_selected_t(:,10)=dataleft(:,20);
data_selected_t(:,11)=dataleft(:,18);
data_selected_t(:,12)=dataleft(:,22);

%[Train, Test] = crossvalind('HoldOut', classall, 0.3);
datatrain_f = data_selected(:,:);
datatest_f=data_selected_t(:,:);
classtrain=classall(:,:);
classtest=classt(:,:);
min=0;
