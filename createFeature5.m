...
for number = ninit:nend
  ImgName = strcat(DirImages,cat(1,ImgList(number).name));
    ...
    %13
    vari=outt.sosvh ;%= zeros(1,size_glcm_3); % Sum of sqaures: Variance [1]
...
dattrain_f = dataall(Train,:);
dattrain=dataall(Train,:);
datatest=dataall(Test,:);
classtrain=classall(Train,:);
classtest=classall(Test,:);

%SVMModels = fitcsvm(dattrain,classtrain,'KernelFunction','rbf','KernelScale','auto', 'Standardize', true,'BoxConstraint',1,'OutlierFraction',0.20);
