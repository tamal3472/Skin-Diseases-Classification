...
fsr=zeros(22,1);
for gg=1:22
    up=27*((((aamuA(1,gg)-aamuall(1,gg))^2)+(aamuB(1,gg)-aamuall(1,gg))^2));
    down=26*(aavarA(1,gg)+aavarB(1,gg));
    fsr(gg,1)=up/down;
end

data_selected=zeros(54,12);
data_selected(:,1:2)=datALL(:,1:2);
data_selected(:,3)=datALL(:,5);
data_selected(:,4:5)=datALL(:,8:9);
data_selected(:,6:11)=datALL(:,12:17);
data_selected(:,12)=datALL(:,20);

[Train, Test] = crossvalind('HoldOut', classall, 0.3);
dattrain_f = data_selected(Train,:);
dattrain=dataall(Train,:);
datatest_f=data_selected(Test,:);
datatest=dataall(Test,:);
classtrain=classall(Train,:);
classtest=classall(Test,:);

SVMModels = fitcsvm(dattrain_f,classtrain,'KernelFunction','rbf','KernelScale','auto', 'Standardize', true,'BoxConstraint',1,'OutlierFraction',0.20);
